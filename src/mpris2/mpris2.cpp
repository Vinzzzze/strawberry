...
TrackMetadata Mpris2::LastPlayedTracksString() const {
  ...
  for (auto&& current_track : playlist_manager_->active()->played_items()) {
    ...
    if (playlist_manager_->active()->ItemByUuId(track_id)) {
      QVariantMap track_map;
...
TrackMetadata Mpris2::GetTracksMetadataString(const QString& trackList) const {
  ...
  for (auto&& current_track_path : tracks) {
    ...
    auto&& track_definition = playlist_manager_->active()->ItemByUuId(track_id);
...
void Mpris2::EnqueueTracks(bool insertFirst, const Track_Ids& tracks) {
  ...
  for (auto&& current_track_id : tracks) {
    ...
    const int track_index = playlist_manager_->active()->IndexByUuId(track_id);
