    ...
    if (filter_tree != nullptr) {
      ...
      for (auto&& current_id : playlist_manager_->active()->virtual_items()) {
        if (   playlist_manager_->active()->has_item_at(current_id)
          ...
          ret_value.push_back(current_track_id(item->uuid()));
        }
      }
      ...
      for (auto&& current_id : playlist_manager_->active()->virtual_items()) {
        if (   playlist_manager_->active()->has_item_at(current_id)
          ...
          if (playlist_manager_->active()->has_item_at(current_id)) {
            ...
            track_map.insert(u"mpris:trackid"_s, QVariant::fromValue(current_track_id(track_definition->uuid())));
...
Track_Ids Mpris2::LastPlayedTracks() const {
  ...
  for (auto&& current_track : playlist_manager_->active()->played_items()) {
     ret_value.push_back(current_track_id(current_track->uuid()));
  }
  return ret_value;
}
...
TrackMetadata Mpris2::LastPlayedTracksString() const {
  ...
  for (auto&& current_track : playlist_manager_->active()->played_items()) {
    auto&& track_id = current_track->uuid();
    if (playlist_manager_->active()->find_item_id(track_id)) {
      QVariantMap track_map;
      current_track->EffectiveMetadata().ToXesam(&track_map);
      track_map.insert(u"mpris:trackid"_s, QVariant::fromValue(current_track_id(current_track->uuid())));
      ret_value.push_back(track_map);
    }
  }
  return ret_value;
...
TrackMetadata Mpris2::GetTracksMetadataString(const QString& trackList) const {
  ...
  for (auto&& current_track_path : tracks) {
    QUuid track_id = QUuid::fromString(current_track_path.replace(u'_', u'-'));
    auto&& track_definition = playlist_manager_->active()->find_item_id(track_id);
...
void Mpris2::EnqueueTracks(bool insertFirst, const Track_Ids& tracks) {
  ...
  for (auto&& current_track_id : tracks) {
    ...
    const QUuid track_id = QUuid::fromString(split_path.last().replace(u'_', u'-'));
    const int track_index = playlist_manager_->active()->find_pos_id(track_id);

    if (track_index >= 0) {
      indexList.push_back(playlist_manager_->active()->index(track_index, 0));
    }
  }
...
void Mpris2::EnqueueTracksString(bool insertFirst, const QString& trackList) {
  ...
  for (auto&& current_track_id : tracks) {
    const QUuid track_id = QUuid::fromString(current_track_id.replace(u'_', u'-'));
    const int track_index = playlist_manager_->active()->find_pos_id(track_id);
