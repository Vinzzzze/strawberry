...
bool AlbumShuffleComparator(const QHash<QString, int>& album_key_positions,
                            const QHash<int, QString> &album_keys,
                            const int left,
                            const int right) {
...
void Playlist::ReshuffleIndices() {
  ...
  switch (shuffle_mode) {
    ...
    case PlaylistSequence::ShuffleMode::Albums:{
      ...
      const auto comparator = [&album_key_positions, &album_keys](int lhs, int rhs) {
        return AlbumShuffleComparator(album_key_positions, album_keys, lhs, rhs);
      };
      std::stable_sort(virtual_items_.begin(), virtual_items_.end(), comparator);
    ...
    case PlaylistSequence::ShuffleMode::Grouping:{
      ...
      const auto comparator = [&grouping_key_positions, &grouping_keys](int lhs, int rhs) {
